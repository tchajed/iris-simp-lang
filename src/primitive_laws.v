...
The state interpretation for simp_lang maps `gmap loc val` into an appropriate RA. Most of the definition is related to `gen_heapG`, which is defined in `heap_ra.v`. We can think of the state interpretation as being an invariant maintained by the weakest precondition, except that it is a function of the state and thus has meaning tied to the program execution. Therefore we pick an RA which is like an auth of a gmap for the state interpretation and map `σ : gmap loc val` to something like `own γ (●σ)`. Then we can use fragments to define the core points-to connective for this language, something like `l ↦ v := own γ (◯{|l := v|})`.
...
The code implements this with two differences. First, the RA we actually use in `heap_ra.` is an Iris library `gmap_viewR loc val` which uses a generalization of auth called views. The important point is that the auth component is the state's heap and the fragments are sub-heaps; the view RA keeps track of the fact that the composition of all the fragments is a sub-heap of the authoritative element. It also adds something called discardable fractions, a generalization of the fraction RA, in order to model fractional and persistent permissions to heap locations, but we can mostly ignore this complication.

Second, there's a pesky ghost name `γ` in the informal definitions above. These are hidden away in the `simpG` typeclass as part of `gen_heapG` that all proofs about this language will carry. It'll be fixed once before any execution by the adequacy theorem, as you'll see in adequacy.v. After that we get it through a typeclass to avoid mentioning it explicitly in any proofs.

If you were writing your own language, you would probably start with `gen_heapG` from the Iris standard library to get all the nice features and lemmas for the heap part of the state interpretation. Then you could add other algebras and global ghost names to the equivalent of `simpG`, as long as you also instantiate them in `adequacy.v`.
|*)
