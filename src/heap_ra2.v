...
Section heap_map.
  ...
  (* no persistent ownership makes sense for this RA (technically Ok {| auth :=
  None; frag := ∅ |} can be owned persistently, but such an element is
  useless) *)
  ...
  Lemma heap_map_unit_ok (hm: heap_map) : hm ⋅ ε = hm.
  Proof.
    rewrite /ε /heap_map_unit.
    rewrite /op /heap_map_op_instance.
    destruct hm as [[[m|] m'] | ]; auto.
    - rewrite -> decide_True by apply map_disjoint_empty_r.
  ...
  Lemma heap_map_included mm1 m1' mm2 m2' :
    Ok {| auth := mm1; frag := m1'; |} ≼ Ok {| auth := mm2; frag := m2'; |} ↔
    (∀ m1, mm1 = Some m1 → mm2 = mm1) ∧
    m1' ⊆ m2'.
  Proof.
    rewrite /Frag /Auth /=.
    rewrite /included /op /heap_map_op_instance /=.
    split.
    - intros [z Heq].
      rewrite leibniz_equiv_iff in Heq.
      destruct z as [[[m3|] m3']|].
      + destruct mm1; try congruence.
        revert Heq; destruct (decide _); intros [=]; (intuition subst); try congruence.
      ...
      + destruct mm1; try congruence.
        ...
        * revert Heq; destruct (decide _); intros [=]; (intuition subst); try congruence.
          apply map_union_subseteq_l.
      + destruct mm1; congruence.
    - intros [Hauth_eq Hsub].
      exists (Ok {| auth := match mm1 with Some m => None | None => mm2 end;
                   frag := m2' ∖ m1';
                 |}).
      rewrite leibniz_equiv_iff.
      destruct mm1 as [m1|].
      + specialize (Hauth_eq _ eq_refl); subst.
        rewrite decide_True.
        2: {
  ...
  Lemma heap_map_invalid_included hm : Invalid ≼ hm ↔ hm = Invalid.
  Proof.
    rewrite /included /op /heap_map_op_instance.
    split.
    - intros [z Heq]; rewrite leibniz_equiv_iff in Heq; auto.
    - intros ->.
      exists Invalid; auto.
  Qed.

  Lemma heap_map_frag_included m m1' m2' : Frag m1' ≼ Auth m ⋅ Frag m2' ↔ m1' ⊆ m2'.
  Proof.
    rewrite /Frag /Auth /=.
    rewrite heap_map_op_auth_frag.
    rewrite heap_map_included.
    intuition eauto.
    congruence.
  Qed.

  Lemma heap_map_auth_included m1 m2 : Auth m1 ≼ Auth m2 ↔ m1 = m2.
  Proof.
    rewrite /Frag /Auth /=.
    rewrite heap_map_included.
  ...
  Definition heap_map_ucmra_mixin : UcmraMixin heap_map.
  Proof.
    split.
    - cbv; auto.
    - intros hm.
      rewrite heap_map_op_comm heap_map_unit_ok //.
    - reflexivity.
  Qed.
  Canonical Structure heap_mapUR := Ucmra (heap_map) heap_map_ucmra_mixin.
